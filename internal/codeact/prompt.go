...
func GenerateMain(plan schema.Plan, history []Attempt) (string, error) {
	...
	if plan.Source == "json" && plan.Target == "xml" && plan.Operation == "convert" {
		if strings.TrimSpace(plan.InputPath) == "" || strings.TrimSpace(plan.OutputPath) == "" {
			return "", errors.New("json to xml plan requires input and output paths")
		}
		return generateJsonToXml(plan)
	}
...
func generateXmlToJson(plan schema.Plan) (string, error) {
	...
	b.WriteString("func xmlToMap(d *xml.Decoder) (map[string]any, error) {\n")
	b.WriteString("\tresult := make(map[string]any)\n")
	b.WriteString("\tfor {\n")
	b.WriteString("\t\ttoken, err := d.Token()\n")
	b.WriteString("\t\tif err == io.EOF { break }\n")
	b.WriteString("\t\tif err != nil { return nil, err }\n")
	b.WriteString("\t\tswitch t := token.(type) {\n")
	b.WriteString("\t\tcase xml.StartElement:\n")
	b.WriteString("\t\t\tvar val any\n")
	b.WriteString("\t\t\tkey := t.Name.Local\n")
	b.WriteString("\t\t\tnode := make(map[string]any)\n")
	b.WriteString("\t\t\tfor _, attr := range t.Attr {\n")
	b.WriteString("\t\t\t\tnode[attr.Name.Local] = attr.Value\n")
	b.WriteString("\t\t\t}\n")
	b.WriteString("\t\t\tchild, err := xmlToMap(d)\n")
	b.WriteString("\t\t\tif err != nil { return nil, err }\n")
	b.WriteString("\t\t\tvar finalChild any = child\n")
	b.WriteString("\t\t\tif len(child) == 1 && child[\"#text\"] != nil {\n")
	b.WriteString("\t\t\t\tfinalChild = child[\"#text\"]\n")
	b.WriteString("\t\t\t}\n")
	b.WriteString("\t\t\tif finalChild != nil {\n")
	b.WriteString("\t\t\t\tif len(node) == 0 {\n")
	b.WriteString("\t\t\t\t\tval = finalChild\n")
	b.WriteString("\t\t\t\t} else {\n")
	b.WriteString("\t\t\t\t\tnode[\"value\"] = finalChild\n")
	b.WriteString("\t\t\t\t\tval = node\n")
	b.WriteString("\t\t\t\t}\n")
	b.WriteString("\t\t\t} else {\n")
	b.WriteString("\t\t\t\tval = node\n")
	b.WriteString("\t\t\t}\n")
	b.WriteString("\t\t\tif existing, ok := result[key]; ok {\n")
	b.WriteString("\t\t\t\tif list, ok := existing.([]any); ok {\n")
	b.WriteString("\t\t\t\t\tresult[key] = append(list, val)\n")
	b.WriteString("\t\t\t\t} else {\n")
	b.WriteString("\t\t\t\t\tresult[key] = []any{existing, val}\n")
	b.WriteString("\t\t\t\t}\n")
	b.WriteString("\t\t\t} else {\n")
	...
	b.WriteString("\t\t\tcontent := strings.TrimSpace(string(t))\n")
	b.WriteString("\t\t\tif content != \"\" {\n")
	b.WriteString("\t\t\t\tresult[\"#text\"] = content\n")
	b.WriteString("\t\t\t}\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t}\n")
	b.WriteString("\treturn result, nil\n")
	...
	b.WriteString("\tdecoder := xml.NewDecoder(strings.NewReader(string(data)))\n")
	b.WriteString("\tvar root map[string]any\n")
	b.WriteString("\tfor {\n")
	b.WriteString("\t\ttoken, err := decoder.Token()\n")
	b.WriteString("\t\tif err != nil { break }\n")
	b.WriteString("\t\tif start, ok := token.(xml.StartElement); ok {\n")
	b.WriteString("\t\t\tval, err := xmlToMap(decoder)\n")
	b.WriteString("\t\t\tif err != nil { return err }\n")
	b.WriteString("\t\t\tvar finalVal any = val\n")
	b.WriteString("\t\t\tif len(val) == 1 && val[\"#text\"] != nil {\n")
	b.WriteString("\t\t\t\tfinalVal = val[\"#text\"]\n")
	b.WriteString("\t\t\t}\n")
	b.WriteString("\t\t\troot = map[string]any{start.Name.Local: finalVal}\n")
	b.WriteString("\t\t\tbreak\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t}\n\n")

	b.WriteString("\tjsonData, err := json.MarshalIndent(root, \"\", \"  \")\n")
	b.WriteString("\tif err != nil { return err }\n\n")
...
func generateJsonToXml(plan schema.Plan) (string, error) {
	...
	b.WriteString("func marshalXML(v any, name string) string {\n")
	b.WriteString("\tswitch val := v.(type) {\n")
	b.WriteString("\tcase map[string]any:\n")
	b.WriteString("\t\tvar res string\n")
	b.WriteString("\t\tres += \"<\" + name + \">\\n\"\n")
	b.WriteString("\t\tfor k, v := range val {\n")
	b.WriteString("\t\t\tres += marshalXML(v, k)\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t\tres += \"</\" + name + \">\\n\"\n")
	b.WriteString("\t\treturn res\n")
	b.WriteString("\tcase []any:\n")
	b.WriteString("\t\tvar res string\n")
	b.WriteString("\t\tfor _, item := range val {\n")
	b.WriteString("\t\t\tres += marshalXML(item, name)\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t\treturn res\n")
	b.WriteString("\tdefault:\n")
	b.WriteString("\t\treturn \"<\" + name + \">\" + fmt.Sprintf(\"%v\", val) + \"</\" + name + \">\\n\"\n")
	b.WriteString("\t}\n")
	b.WriteString("}\n\n")
	...
	b.WriteString("\txmlContent += marshalXML(value, \"root\")\n\n")

	b.WriteString("\tif err := os.WriteFile(outputPath, []byte(xmlContent), 0644); err != nil { return err }\n")
	b.WriteString("\treturn nil\n")
	b.WriteString("}\n\n")
